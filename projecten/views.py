...
class UrenMedewerkerView(View):
    ...
    def get(self, request):
        ...
        for order in orders:
            ...
            maandlijst = MAANDLIJST.copy()
            # Voor iedere order haal omzetten op
            urenlist = Urenpermedewerker.objects.filter(projectcode__id=order.id).filter(jaar=maand_in_jaar)
            # Voor iedere maand vul de juiste omzet per maand in
            if len(urenlist) > 0:
                for uren in urenlist:
                    maandlijst[uren.maand - 1] = uren.uren
                    uren_totaal = uren_totaal + uren.uren

                    try:
                        if uren.jaar == maand_in_jaar:
                            uren_per_maand_item = collections.OrderedDict()
                            uren_per_maand_item['Verkoopstadium'] = str(order.verkoopstadium)
                            uren_per_maand_item['Productgroup'] = str(order.productgroep)
                            uren_per_maand_item['Projectcode'] = order.projectcode
                            uren_per_maand_item['Klant'] = str(order.bedrijf)
                            uren_per_maand_item['Branche'] = order.bedrijf.branche.branch
                            uren_per_maand_item['opdrachtgever'] = str(order.opdrachtgever)
                            uren_per_maand_item['Omschrijving'] = order.omschrijving
                            uren_per_maand_item['Klantpartner'] = str(order.klantpartner)
                            uren_per_maand_item['Ordereigenaar'] = str(order.ordereigenaar)
                            uren_per_maand_item['Medewerker'] = str(uren.medewerker)
                            uren_per_maand_item['Product'] = str(uren.product).rsplit(" ")[0]
                            uren_per_maand_item['Januari'] = float(maandlijst[0])
                            uren_per_maand_item['Februari'] = float(maandlijst[1])
                            uren_per_maand_item['Maart'] = float(maandlijst[2])
                            uren_per_maand_item['April'] = float(maandlijst[3])
                            uren_per_maand_item['Mei'] = float(maandlijst[4])
                            uren_per_maand_item['Juni'] = float(maandlijst[5])
                            uren_per_maand_item['Juli'] = float(maandlijst[6])
                            uren_per_maand_item['Augustus'] = float(maandlijst[7])
                            uren_per_maand_item['September'] = float(maandlijst[8])
                            uren_per_maand_item['Oktober'] = float(maandlijst[9])
                            uren_per_maand_item['November'] = float(maandlijst[10])
                            uren_per_maand_item['December'] = float(maandlijst[11])
                            uren_per_maand_item['Year end'] = float(uren_totaal)

                            uren_per_maand.append(uren_per_maand_item)
                    except:
                        print('Niets gevonden!')

        return JsonResponse(uren_per_maand, json_dumps_params={'indent': 3}, safe=False)
